...
class InputBox(Entity):
    ...
    def __init__(self, parent, position, size, text=''):
        ...
        rectRenderer = RectRenderer(self.screen, self.color_inactive, self.rect.size, int(size[1]/15), int(size[1]/5))
        self.rectEntity = Entity(self, position)
        self.rectEntity.set_renderer(rectRenderer)

        textRenderer = TextRenderer(self.screen, (0,0,0), text, self.rect.size[1])
    ...
    def recalc_props(self):
        (self.rect.x, self.rect.y) = self.local_position
